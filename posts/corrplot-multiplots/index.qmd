...
Second, let's get some data, here from the `{palmerpenguins}` package that provides anthropometric characteristics of three species of penguins:

```{r}
data("penguins")
```

Third, let's compute correlation matrices putting into relation the anthropometric characteristics for each of the three species of penguins:

```{r}
# Matrix correlation for 'Adelie' specie
mat_adelie <- 
  penguins %>% 
...
# Matrix correlation for 'Gentoo' specie
mat_gentoo <- 
  penguins %>% 
...
# Matrix correlation for 'Chinstrap' specie
mat_chinstrap <- 
  penguins %>% 
...
The reason is that the plot is made "invisible" when it is provided by the `corrplot()` function, making it difficult to catch. 
